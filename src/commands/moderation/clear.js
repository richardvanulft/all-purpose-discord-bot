...
module.exports = {
  ...
  async interactionRun(interaction) {
    ...
    if (amount === "*") {
      ...
      do {
        batch = await interaction.channel.bulkDelete(100, true).catch(err => {
          if (err.message.includes("You can only bulk delete messages that are under 14 days old.")) {
            interaction.followUp("I cannot delete messages that are older than 14 days.");
          } else {
      ...
      if (deleted) interaction.followUp(`Successfully deleted ${deleted} messages.`);
    } else {
      // Delete specified amount of messages
      const count = parseInt(amount);
      if (isNaN(count) || count < 1 || count > 100) {
        interaction.followUp("You need to input a number between 1 and 100.");
      } else {
